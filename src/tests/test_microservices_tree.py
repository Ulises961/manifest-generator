...
def test_prepare_microservice_with_volume(tree):
    ...
    if len(result["volumes"]) > 0:
        assert result["volume_mounts"][0] == {"name": "volume-0", "mountPath": "/data"}
...
def test_prepare_microservice_with_annotations(tree):
    ...
    annotation_node = Node(
        name="annotation", type=NodeType.ANNOTATION, value="key=value"
    )
...
def test_prepare_microservice_with_entrypoint(tree):
    ...
    entrypoint_node = Node(
        name="entrypoint", type=NodeType.ENTRYPOINT, value="python app.py"
    )
...
def test_prepare_microservice_with_secrets(tree):
    ...
    assert result["secrets"] == [
        {"name": "DB_PASSWORD", "key": "password", "value": "password123"}
    ]


def test_prepare_microservice_with_service_classifier_ports(
    tree, mock_service_classifier
):
    mock_service_classifier.decide_service.return_value = {
        "workload": "Deployment",
        "protocol": "TCP",
