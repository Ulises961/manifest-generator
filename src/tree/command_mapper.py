...
class CommandMapper:
    ...
    def get_commands(self, parsed_dockerfile: List[dict]) -> List[DockerInstruction]:
        ...
        return [
            self.generate_node_from_command(command)
            for command in parsed_dockerfile
        ]

    def generate_node_from_command(self, command: dict) -> DockerInstruction:
        ...
        return switch.get(instruction, lambda x: None)(command)

    def _generate_cmd_node(self, command: dict) -> DockerInstruction:
        """Generate a node from a CMD command."""
        return self._create_docker_node(command, NodeType.CMD)

    def _generate_label_node(self, command: dict) -> DockerInstruction:
        """Generate a node from a LABEL command."""
        is_label = self.decide_label(command["value"])

        return self._create_docker_node(command, NodeType.LABEL if is_label else NodeType.ANNOTATION)
      

    def _generate_expose_node(self, command: dict) -> DockerInstruction:
        """Generate a node from an EXPOSE command."""
        return {"type": "PORTS", "command": command["value"]}
    
    def _generate_entrypoint_node(self, command: dict) -> DockerInstruction:
        """Generate a node from an ENTRYPOINT command."""
        return self._create_docker_node(command, NodeType.ENTRYPOINT)
    
    def _generate_volume_node(self, command: dict) -> DockerInstruction:
        """Generate a node from a VOLUME command."""
        # Check if the volume is persistent
        with open(os.path.join(os.path.dirname(__file__), os.getenv("VOLUMES_PATH")), "r") as f:
            volumes = json.load(f)

        is_persistent = command["value"] in volumes 
        return self._create_docker_node(command, NodeType.VOLUME, is_persistent=is_persistent)
    
    def _generate_user_node(self, command: dict) -> DockerInstruction:
        """Generate a node from a USER command."""
        return self._create_docker_node(command, NodeType.USER)
    
    def _generate_workdir_node(self, command: dict) -> DockerInstruction:
        """Generate a node from a WORKDIR command."""
        return self._create_docker_node(command, NodeType.WORKDIR)
    
    def _generate_healthcheck_node(self, command: dict) -> DockerInstruction:
        """Generate a node from a HEALTHCHECK command."""
        return self._create_docker_node(command, NodeType.HEALTHCHECK)
    
    def _generate_stopsignal_node(self, command: dict) -> DockerInstruction:
        """Generate a node from a STOPSIGNAL command."""
        return self._create_docker_node(command, NodeType.STOPSIGNAL)
    ...
    def _create_docker_node(self, command: dict, type: NodeType) -> DockerInstruction:
        """Create a DockerInstruction from a command."""
        node = self.generate_node_from_command(command)
        return DockerInstruction(
            name=node["instruction"],
            type=type,
            command=node["command"],
            parent=None,
            is_persistent=node.get("is_persistent", False),
        )
